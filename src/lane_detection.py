...
def polynomial_fitting(warped_image, H, cropped_image):
	...
	if left_curve_radius <= 1500:
		print('turining left')
		cv2.putText(super_imposed, "turining left", (100,80), cv2.FONT_HERSHEY_SIMPLEX, 0.75,(0,0,255),2)

	elif left_curve_radius >1500 and left_curve_radius <= 3500:
		print('moving straight')
		cv2.putText(super_imposed, "moving straight", (100,80), cv2.FONT_HERSHEY_SIMPLEX, 0.75,(0,0,255),2)

	else:
		print('turning right')
		cv2.putText(super_imposed, "turning right", (100,80), cv2.FONT_HERSHEY_SIMPLEX, 0.75,(0,0,255),2)

	cv2.imshow('Lane Detected', super_imposed)

	print('shape is ',super_imposed.shape)
...
while cap.isOpened():
	...
	#####################  Polynomial Fitting   ####################
	output_img = polynomial_fitting(warped_image,H_matrix,cropped_frame)
	Out_list.append(output_img)
...
out = cv2.VideoWriter('Lane_output.avi', source, 5.0, (300,1240))
for image in Out_list:
    out.write(image)
    cv2.waitKey(10)
out.release()

# print('total images',len(Out_list))
#
# i = 0
# for im in Out_list:
# 	print(i)
